...
export class PunkMemoryOptimizer extends EventEmitter {
    constructor() {
        ...
        this.leakThreshold = 0.99; // 🔥 MODIFIED: 99% threshold for leak detection (increased from 98%)
     ...
     */
    detectMemoryLeak(current) {
        if (!this.baselineMetrics)
            return false;
        const heapUsageRatio = current.heapUsed / current.heapTotal;
        const growthRate = (current.heapUsed - this.baselineMetrics.heapUsed) /
    ...
    handleMemoryLeak(metrics) {
        ...
        console.log(`🚨 CRITICAL MEMORY LEAK: Heap at ${(heapUsageRatio * 100).toFixed(1)}% (${(metrics.heapUsed / 1024 / 1024).toFixed(1)}MB used)`);
    ...
    getDetailedStats() {
        ...
        heapSpaceStats.forEach((space) => {
            if (space.space_used_size > space.space_size * 0.9) {
                possibleLeaks.push(`${space.space_name}: ${Math.round(space.space_used_size / 1024 / 1024)}MB used`);
            }
        });
        return {
     ...
     */
    breakCircularReferences(obj, seen = new Set()) {
     ...
     */
    async performFullSystemOptimization() {
...
//# sourceMappingURL=PunkMemoryOptimizer.js.map