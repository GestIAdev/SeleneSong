...
async function waitForRedis(maxRetries = 10, retryDelayMs = 2000): Promise<any> {
  // NO usar logger aqu� - bucle infinito
  let attempt = 0;
  ...
  while (attempt < maxRetries) {
    try {
      ...
      // ?? SANITACI�N-QUIR�RGICA: Unique client per node (eliminates race condition)
      // Pattern: selene-startup-selene-node-1/2/3 (from PM2 NODE_ID env var)
      const primaryClient = redis.getRedisClient(`selene-startup-${process.env.NODE_ID || `node-${process.pid}`}`);
      
      if (!primaryClient) {
        throw new Error('Primary Redis client not initialized');
      }
      ...
      return primaryClient; // Return connected client for reuse
    } catch (error) {
      attempt++;
...
/**
 * ??? MAIN EXECUTION - V167 WITH SANITACI�N-TOTAL
 */
async function main() {
  // ============================================================================
  // STEP 0: Startup logging (TIERRA QUEMADA - console direct)
  // ============================================================================
  console.log('[STARTUP]', `🚀 Selene starting in ${process.env.NODE_ENV || 'development'} mode [pid=${process.pid}]`);
  ...
  // Logger eliminated - using console directly
  ...
  // ============================================================================
  // ?? V166: Activar silenciador para startup limpio
  if (process.env.APOLLO_VERBOSE_STARTUP !== "true") {
    consoleSilencer.activate();
  }

  // ?? SANITACI�N-QUIR�RGICA: ASCII art banner moved to DEBUG (Bug #4 - console.log anarchists)
  console.log('?? SELENE SONG - The Goddess Awakens | By PunkGrok & RaulVisionario | Mission: Dental AI Empire �90/month');
  ...
  console.log("?? Initializing Punk Memory Optimizer...");
  // PunkMemoryOptimizer is a singleton that auto-initializes in constructor
  console.log(
    "? Punk Memory Optimizer activated - heap anchoring prevention enabled",
  );
  ...
  const createHeapSnapshot = function (_reason = "manual") {
    try {
      ...
      if (
        ...
        console.log(
          "?? Inspector not available for heap snapshot (run with --inspect flag)",
        );
        return false;
      }
      ...
      console.log("?? Heap snapshot not available:", (error as Error).message);
      return false;
    }
  };
  ...
  const runMemoryForensics = function () {
    console.log("\n?? ===== SELENE SONG CORE MEMORY FORENSICS =====");
    ...
    // ?? AUTO SNAPSHOT ON CRITICAL MEMORY
    if (
      hasProblemSpaces ||
      ...
      try {
        ...
        if (obj && typeof obj === "object") {
          ...
          if (Array.isArray(obj) && obj.length > 100) {
            console.log("    ?? LARGE ARRAY: " + obj.length + " items");
          }
          if (obj instanceof Map && obj.size > 50) {
            console.log("    ?? LARGE MAP: " + obj.size + " entries");
          }
        }
      } catch (e) {
    ...
    if (memoryTimeline.length >= 3) {
      ...
      if (heapGrowth > 10) {
        console.log(
          "  ?? SIGNIFICANT HEAP GROWTH DETECTED! Creating heap snapshot...",
        );
    ...
    if (memoryTimeline.length % 12 === 0 && memoryTimeline.length > 0) {
      ...
      console.log("  ?? Creating periodic heap snapshot...");
  ...
  // ?? CONTINUOUS CRITICAL MEMORY MONITOR
  ...
  function checkHeapExpansionHealth() {
    ...
    // ?? CRITICAL FIX: REAL heap anchoring occurs when:
    ...
    if (isReallyBlocked) {
      ...
      console.log(
        `?? REAL HEAP ANCHORING: ${heapUsagePercent.toFixed(1)}% usage in ${Math.round(heapStats.total_heap_size / 1024 / 1024)}MB heap (vs ${Math.round(heapStats.heap_size_limit / 1024 / 1024)}MB limit)`,
      );

      if (heapExpansionBlockedCount >= maxBlockedExpansions) {
        console.log(
          "?? CRITICAL: True heap anchoring detected - forcing cleanup",
        );
        ...
        if (Array.isArray(memoryTimeline) && memoryTimeline.length > 2) {
          ...
          console.log(
            "?? Memory timeline aggressively cleaned - kept only last entry",
          );
        }
      ...
      if (
        ...
        console.log(
          `? HEAP EXPANDING NATURALLY: ${Math.round(heapStats.total_heap_size / 1024 / 1024)}MB total, ${heapUsagePercent.toFixed(1)}% usage`,
        );
      }
    }
  }
  ...
  function takeEmergencySnapshot(_reason: string) {
    if (emergencySnapshotCount >= maxEmergencySnapshots) {
      console.log("?? Emergency snapshot limit reached, skipping...");
      return;
    }
    ...
    try {
      ...
      if (
        ...
        console.log(
          "?? Inspector not available for emergency snapshot (run with --inspect flag)",
        );
        return;
      }
      ...
      session.post("HeapProfiler.enable", () => {
        ...
        session.post(
          ...
          (err: any) => {
            if (err) {
              console.error("? Emergency heap snapshot failed:", err);
            } else {
              console.log(
                `?? ?? ?? EMERGENCY SNAPSHOT TAKEN: ${filename} ?? ?? ??`,
              );
      ...
      console.log(
        "?? Emergency snapshot not available:",
  ...
  // ?? MISSION V407: 5-minute 'checkCriticalMemory' monitor removed by GeminiPunk.

  console.log(
    "?? Emergency memory monitor activated - checking every 5 minutes",
  );

  // ?? AGGRESSIVE HEAP MONITORING - IMMEDIATE CLEANUP AT 90%
  let emergencyCleanupCount = 0;
  const MAX_EMERGENCY_CLEANUPS = 5; // ?? AGGRESSIVE LIMIT: Max 5 emergency cleanups per session

  function performEmergencyCleanup(_reason: string) {
    if (emergencyCleanupCount >= MAX_EMERGENCY_CLEANUPS) {
      console.log(
        "?? Emergency cleanup limit reached - system may need restart",
      );
      return false;
    }
    emergencyCleanupCount++;

    console.log(`?? EMERGENCY CLEANUP #${emergencyCleanupCount}: ${_reason}`);
    ...
    if (global.gc) {
      ...
      totalFreed += gcFreed;
      console.log(
        `?? GC: Freed ${Math.round((gcFreed / 1024 / 1024) * 100) / 100}MB`,
      );
    }
    ...
    if (Array.isArray(memoryTimeline) && memoryTimeline.length > 2) {
      ...
      console.log(`?? Memory timeline: Removed ${removed} old entries`);
    }
      ...
      try {
        ...
        if (obj && Array.isArray(obj) && obj.length > 1000) {
          ...
          if (
            ...
            clearedObjects++;
            console.log(
              `?? ${key}: Reduced from ${originalLength} to 100 items`,
            );
          }
        }
      } catch (e) {
    ...
    if (clearedObjects > 0) {
      console.log(`?? Cleared ${clearedObjects} large global arrays`);
    }
    ...
    console.log(
      `? Emergency cleanup complete: Freed ${Math.round((totalFreed / 1024 / 1024) * 100) / 100}MB total`,
    );
    console.log(
      `?? Heap after cleanup: ${Math.round(afterCleanup.heapUsed / 1024 / 1024)}MB used`,
    );

    return totalFreed > 0;
  }

  // ?? MISSION V407: Aggressive 30-second memory monitor removed by GeminiPunk.
  ...
  try {
    ...
    console.log("? configureGraphQL() completed successfully");

    // ?? V166: Desactivar silenciador y mostrar resumen
    if (process.env.APOLLO_VERBOSE_STARTUP !== "true") {
    ...
    console.log("?? /api/v1/auth/login  READY");
    console.log("?? /api/v1/auth/me  READY");
    console.log("?? /graphql  READY WITH @VERITAS");
    console.log("?? /health  READY");
    console.log("?? Frontend can now authenticate for �90/month");
    console.log("?? GraphQL queries with @veritas(level: CRITICAL) available");

    // Keep the process alive
    console.log("?? Selene Song Core running... Press Ctrl+C to stop");
  } catch (error) {
    console.error("? Selene Song Core startup failed:", error);
    process.exit(1);
  }
}
...
main().catch((_error) => {
  console.error("?? Critical startup error:", _error);
  process.exit(1);
});

// ?? CLUSTER-WIDE MEMORY MONITORING - IDENTIFY ANCHORING COMPONENTS
const MAX_MEMORY_CHECKS = 10; // ?? AGRESSIVE LIMIT: Max 10 memory checks to prevent log spam
let memoryCheckCount = 0;

function performClusterMemoryAudit() {
  if (memoryCheckCount >= MAX_MEMORY_CHECKS) {
    console.log(
      "?? Cluster memory audit limit reached - stopping automated checks",
    );
    return;
  }
  memoryCheckCount++;

  console.log("\n?? ===== CLUSTER MEMORY AUDIT =====");
  ...
  console.log(
    `?? HEAP STATUS: ${heapUsagePercent.toFixed(1)}% used (${Math.round(heapStats.used_heap_size / 1024 / 1024)}MB/${Math.round(heapStats.total_heap_size / 1024 / 1024)}MB)`,
  );

  // Check if we're in critical anchoring territory
  if (heapUsagePercent > 90) {
    console.log("?? CRITICAL: Heap usage >90% - performing deep memory audit");
    ...
    if (largeObjects.length > 0) {
      console.log("?? LARGE MEMORY OBJECTS DETECTED:");
      ...
      console.log("? No large memory objects found in global scope");
    }
    ...
    if (global.gc) {
      ...
      const freed = beforeGC.heapUsed - afterGC.heapUsed;
      console.log(
        `?? GC Result: Freed ${Math.round((freed / 1024 / 1024) * 100) / 100}MB`,
      );
    }
