...
import {
    InstrumentSelection,
    StylePreset,
    SonicPalette,    // 🎸 FASE 5.9
    VibeType,        // 🎸 FASE 5.9
    InstrumentRole   // 🎸 FASE 5.9
} from '../style/StylePreset.js'


// 🎨 SCHERZO SÓNICO - Fase 4.1: Metadata de Track para JSON
...
export class MusicEnginePro {
    ...
    private melodicLayers: InstrumentSelection[] = []
    ...
    async generate(
        ...
        for (const section of structure.sections) {
            ...
            const layers = this.orchestrator.generateLayers(
                section,
                resolvedChords,
                melodyLayers[0], // ✅ Pasar primera capa como referencia (Orchestrator legacy)
                modifiedStyle,
    ...
    private selectMelodicLayers(
        ...
        const selectedLayers: InstrumentSelection[] = []
        const usedPoolNames: string[] = []
        ...
        for (let i = 0; i < numLayers; i++) {
            ...
            if (availablePools.length === 0) break // No hay más pools disponibles
            ...
            const pool = pools[selectedPoolName as keyof typeof pools]
