...
export class VoiceLeading {
     ...
     */
    private static applySmoothVoiceLeading(fromChord: MIDINote[], toChord: MIDINote[]): MIDINote[] {
     ...
     */
    private static applyContraryVoiceLeading(fromChord: MIDINote[], toChord: MIDINote[]): MIDINote[] {
     ...
     */
    private static applyParallelVoiceLeading(fromChord: MIDINote[], toChord: MIDINote[]): MIDINote[] {
     ...
     */
    private static applyObliqueVoiceLeading(fromChord: MIDINote[], toChord: MIDINote[]): MIDINote[] {
        const result: MIDINote[] = []
     ...
     */
    static optimizeChordSequence(
        chords: MIDINote[][],
        strategy: VoiceLeadingStrategy = 'smooth'
    ): MIDINote[][] {
        if (chords.length <= 1) return chords

        const optimized: MIDINote[][] = [chords[0]] // Primer acorde sin cambios

        for (let i = 1; i < chords.length; i++) {
            const optimizedChord = this.minimizeVoiceMovement(
                optimized[i - 1],
                chords[i],
                strategy
            )
