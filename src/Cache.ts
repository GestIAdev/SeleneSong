...
export class SeleneCache {
  ...
  private async ensureConnection(): Promise<void> {
    try {
      ...
      console.warn("⚠️ Redis reconnection failed, continuing in offline mode");
      this.isConnected = false;
    }
  }
  ...
  public async clear(): Promise<void> {
    try {
      ...
      if (this.isConnected && this.client.isReady) {
        // 🧠 SELECTIVE CACHE CLEAR: Preserve consciousness, delete everything else
        ...
        for (const key of allKeys) {
          if (key.startsWith('selene:consciousness:')) {
            ...
            continue; // PRESERVE consciousness memory
          }
          keysToDelete.push(key); // DELETE swarm/consensus/temp
        }
        
        // Delete non-consciousness keys
        ...
        console.warn("⚠️ Redis unavailable, clearing local cache only");
      }
