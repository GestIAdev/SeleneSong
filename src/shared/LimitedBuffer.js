...
export class LimitedBuffer {
    constructor(id, options) {
        ...
        console.log(`📦 LimitedBuffer[${this.id}]: Creado con límite ${this.options.maxSize}, modo: ${this.options.onOverflow}`);
    }
    /**
    ...
    push(item) {
        ...
        if (this.buffer.length >=
            ...
            console.warn(`⚠️ LimitedBuffer[${this.id}]: Cerca del límite (${this.buffer.length}/${this.options.maxSize})`);
        }
    ...
    handleOverflow(newItem) {
        ...
        switch (this.options.onOverflow) {
            ...
            default:
                console.error(`❌ LimitedBuffer[${this.id}]: Estrategia de overflow desconocida: ${this.options.onOverflow}`);
                return false;
        }
    }
    /**
    ...
    handleRotateOverflow(_newItem) {
        ...
        this.stats.currentSize = this.buffer.length;
        console.log(`🔄 LimitedBuffer[${this.id}]: Rotación ejecutada (overflow ${this.stats.overflowCount})`);
        return true;
    }
    /**
    ...
    handleRejectOverflow(_newItem) {
        console.log(`❌ LimitedBuffer[${this.id}]: Elemento rechazado por overflow (${this.stats.overflowCount})`);
        return false;
    }
    /**
    ...
    handleCompressOverflow(_newItem) {
        ...
        if (itemsToRemove > 0) {
            ...
            console.log(`🗜️ LimitedBuffer[${this.id}]: Compresión ejecutada, ${itemsToRemove} elementos removidos`);
        }
        this.buffer.push(_newItem);
        this.stats.currentSize = this.buffer.length;
        return true;
    ...
    clear() {
        ...
        this.stats.lastAccess = Date.now();
        console.log(`🧹 LimitedBuffer[${this.id}]: Buffer limpiado (${removedCount} elementos removidos)`);
    }
    /**
    ...
    removeWhere(_predicate) {
        ...
        this.stats.currentSize = this.buffer.length;
        if (removedCount > 0) {
            console.log(`🗑️ LimitedBuffer[${this.id}]: ${removedCount} elementos removidos por condición`);
        }
        return removedCount;
    }
    /**
    ...
    resize(newMaxSize) {
        ...
        if (newMaxSize < this.buffer.length) {
            ...
            console.log(`📏 LimitedBuffer[${this.id}]: Redimensionado de ${oldMaxSize} a ${newMaxSize}, ${itemsToRemove} elementos removidos`);
        }
        else {
            console.log(`📏 LimitedBuffer[${this.id}]: Redimensionado de ${oldMaxSize} a ${newMaxSize}`);
        }
        this.stats.currentSize = this.buffer.length;
    }
    /**
...
export class BufferFactory {
    ...
    static createLogBuffer(_id, _maxSize = 100) {
        return new LimitedBuffer(_id, {
            ...
            onItemRemoved: (_item) => {
                ...
                console.debug(`📝 Log buffer item removed: ${JSON.stringify(_item).substring(0, 100)}...`);
            },
        });
    }
    /**
    ...
    static createEventBuffer(_id, _maxSize = 100) {
        return new LimitedBuffer(_id, {
            ...
            onItemRemoved: (_item) => {
                console.debug(`📅 Event buffer item removed: ${JSON.stringify(_item).substring(0, 50)}...`);
            },
        });
    }
    /**
    ...
    static createCacheBuffer(_id, _maxSize = 200) {
        return new LimitedBuffer(_id, {
            ...
            onItemRemoved: (_item) => {
                console.debug(`💾 Cache buffer item removed: ${JSON.stringify(_item).substring(0, 50)}...`);
            },
        });
    }
    /**
