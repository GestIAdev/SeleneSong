...
export class SeleneDatabase {
  ...
  public async getAppointments(filters?: any): Promise<any[]> {
    ...
    console.log(
      "🔥 PUNK DEBUG: getAppointments() called with filters:",
      filters,
    );
    ...
    try {
      ...
      console.log(
        "🔥 PUNK DEBUG: About to execute query:",
        query.slice(0, 100) + "...",
      );
      ...
      // await this.safeRedisOperation(() => this.getRedis().setEx(cacheKey, 180, JSON.stringify(appointments)));
  ...
  public async getTreatments(filters?: any): Promise<any[]> {
    ...
    try {
      ...
      if (cached) {
        console.log("⚡ Treatments served from cache");
        return JSON.parse(cached);
      }
      ...
      // Cache results (don't fail if Redis is down)
      await this.safeRedisOperation(() =>
        this.getRedis().setEx(cacheKey, 300, JSON.stringify(mappedTreatments)),
      );
  ...
  public async getMedicalRecords(filters?: any): Promise<any[]> {
    ...
    try {
      ...
      // Cache results
      await this.safeRedisOperation(() =>
        this.getRedis().setEx(cacheKey, 600, JSON.stringify(records)),
      );
