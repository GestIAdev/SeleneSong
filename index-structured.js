...
const Server_js_1 = require("./src/core/Server.js");
const server_js_1 = require("./src/graphql/server.js");
const ConsoleSilencer_js_1 = require("./src/ConsoleSilencer.js");
const SeleneLogger_js_1 = require("./src/core/SeleneLogger.js");
...
async function main() {
    ...
    const bootLogger = SeleneLogger_js_1.LoggerFactory.getBootLogger();
    ...
    if (!isVerbose) {
        ConsoleSilencer_js_1.consoleSilencer.activate();
    ...
    try {
        ...
        const server = new Server_js_1.SeleneServer();
        bootLogger.info('Starting server');
        await server.start();
        bootLogger.info('Configuring GraphQL with @veritas directive');
        const graphqlServer = new server_js_1.SeleneNuclearGraphQL(server["database"], server["cache"], server["monitoring"], server["reactor"], server["containment"], server["fusion"], server["veritas"], null, // Consciousness disabled - CPU radiation safety
        server["heal"] || null, server["predict"] || null, server["offline"] || null);
        ...
        if (!isVerbose) {
            ConsoleSilencer_js_1.consoleSilencer.deactivate();
            ConsoleSilencer_js_1.consoleSilencer.showSummary();
        }
...
process.on("SIGINT", () => {
    SeleneLogger_js_1.LoggerFactory.getBootLogger().info('Received SIGINT, shutting down gracefully');
    process.exit(0);
});
process.on("SIGTERM", () => {
    SeleneLogger_js_1.LoggerFactory.getBootLogger().info('Received SIGTERM, shutting down gracefully');
    process.exit(0);
});
// Start the nuclear revolution
main().catch((error) => {
    SeleneLogger_js_1.LoggerFactory.getBootLogger().error('Critical startup error', {
        error: error.message,
        stack: error.stack?.split('\n')[0]
    });
    process.exit(1);
...
function performClusterMemoryAudit() {
    if (memoryCheckCount >= MAX_MEMORY_CHECKS) {
        SeleneLogger_js_1.LoggerFactory.getBootLogger().warn('Cluster memory audit limit reached', {
    ...
    const heapStats = v8.getHeapStatistics();
    const heapUsagePercent = (heapStats.used_heap_size / heapStats.total_heap_size) * 100;
    SeleneLogger_js_1.LoggerFactory.getBootLogger().info('Cluster memory audit', {
    ...
    if (heapUsagePercent > 90) {
        SeleneLogger_js_1.LoggerFactory.getBootLogger().warn('Critical heap usage detected', {
            usagePercent: Math.round(heapUsagePercent * 10) / 10,
...
global.auditClusterMemory = () => {
    SeleneLogger_js_1.LoggerFactory.getBootLogger().debug('Manual cluster memory audit requested');
