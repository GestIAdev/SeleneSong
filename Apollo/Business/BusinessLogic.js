...
export class SeleneBusinessLogic {
    ...
    initializeBusinessLogic() {
        ...
        this.monitoring.logInfo('Selene Business Logic initialized - Rules, workflows, and compliance active');
        console.log('🧠 Selene Business Logic active - Rules → Workflows → Validation → Compliance → Analytics');
    }
    /**
        ...
        const defaultRules = [
            {
                ...
                category: 'validation',
                conditions: { field: 'birth_date', operator: 'age_between', value: [0, 150] },
            ...
            },
            {
                ...
                category: 'compliance',
                conditions: { regulation: 'GDPR', required_fields: ['consent', 'data_processing_purpose'] },
                actions: [{ type: 'warning', message: 'GDPR compliance check required' }],
                priority: 2,
                active: true,
                created_at: new Date(),
                updated_at: new Date()
    ...
    loadDefaultWorkflows() {
        ...
        defaultWorkflows.forEach(workflow => this.workflows.set(workflow.id, workflow));
    }
    /**
    ...
    async validateBusinessRules(request) {
        try {
            ...
            const relevantRules = this.getRelevantRules(request.entity, request.operation);
            for (const rule of relevantRules) {
                if (!rule.active)
                    continue;
                const result = await this.evaluateRule(rule, request.data, request.context);
                if (result.fail) {
    ...
    async executeWorkflow(workflowId, context) {
        try {
            ...
            this.monitoring.logInfo(`Executing workflow: ${workflow.name}`, { context });
            const result = {
     ...
     */
    async checkCompliance(entity, data, regulations = ['GDPR', 'HIPAA']) {
        try {
            const results = [];
            for (const regulation of regulations) {
                const check = await this.performComplianceCheck(regulation, entity, data);
                results.push(check);
            }
            ...
            if (failures.length > 0) {
                this.monitoring.logError('Compliance check failed', {
                    entity,
                    failures: failures.map(f => ({ regulation: f.regulation, details: f.details }))
                });
            }
            return results;
        }
    ...
    async upsertBusinessRule(rule) {
        try {
            ...
            this.monitoring.logInfo(`Business rule ${existingRule ? 'updated' : 'created'}: ${rule.name}`);
            return fullRule;
        }
        catch (error) {
    ...
    async upsertWorkflow(workflow) {
        try {
            ...
            this.monitoring.logInfo(`Workflow ${existingWorkflow ? 'updated' : 'created'}: ${workflow.name}`);
            return fullWorkflow;
        }
        catch (error) {
     ...
     */
    getRelevantRules(entity, operation) {
     ...
     */
    async performComplianceCheck(regulation, entity, data) {
        try {
     ...
     */
    async generatePatientAnalytics(timeRange) {
     ...
     */
    async generateAppointmentAnalytics(timeRange) {
     ...
     */
    async generateRevenueAnalytics(timeRange) {
     ...
     */
    async generateComplianceAnalytics(timeRange) {
    ...
    async getStatus() {
        return {
            ...
            active_rules: Array.from(this.rules.values()).filter(r => r.active).length,
