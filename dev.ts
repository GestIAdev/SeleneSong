...
class MockQueue {
  ...
  async addJob(_queueName: string, _job: any) {
...
async function startDevelopmentMode() {
  ...
  try {
    ...
    (server as any).database = new MockDatabase();
    (server as any).cache = new MockCache();
    (server as any).queue = new MockQueue();
    (server as any).scheduler = new MockScheduler();
    (server as any).monitoring = new MockMonitoring();
    (server as any).reactor = new MockReactor();
    (server as any).radiation = new MockRadiation();
    (server as any).fusion = new MockFusion();
    (server as any).containment = new MockContainment();
