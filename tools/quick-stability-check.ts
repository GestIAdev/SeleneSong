...
async function quickStabilityCheck() {
  ...
  try {
    ...
    console.log(
      `❌ Error en determinismo: ${error instanceof Error ? error.message : String(error)}`,
    );
    results.push({ test: "Determinismo", passed: false });
  }
  ...
  try {
    ...
    console.log(
      `❌ Error en sistema inmune: ${error instanceof Error ? error.message : String(error)}`,
    );
    results.push({ test: "Sistema Inmune", passed: false });
  }
  ...
  try {
    ...
    console.log(
      `❌ Error en coordinador: ${error instanceof Error ? error.message : String(error)}`,
    );
    results.push({ test: "Coordinador Swarm", passed: false });
  }
  ...
  if (Math.abs(memDiff) < 5 * 1024 * 1024) {
    // < 5MB
    console.log(
      `✅ Memoria estable (±${(memDiff / 1024 / 1024).toFixed(2)}MB)`,
    );
    results.push({ test: "Estabilidad Memoria", passed: true });
  } else {
    console.log(
      `❌ Posible fuga de memoria (${(memDiff / 1024 / 1024).toFixed(2)}MB)`,
    );
