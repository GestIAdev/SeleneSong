...
describe("Memory Leak Detection - WeakRef & TTL Cache", () => {
    describe("WeakRef Cleanup Under Stress", () => {
        test("should cleanup WeakRef objects after GC", () => {
            ...
            setTimeout(() => {
                ...
                const dereferenced = refs.filter((ref) => ref.deref() === undefined).length;
        ...
        test("should not leak memory with repeated WeakRef creation", () => {
            ...
            console.log(`💾 Memory growth: ${memoryGrowth.toFixed(2)}MB for 10,000 WeakRefs`);
        });
        test("should allow GC to collect unreferenced objects", async () => {
            let obj = { large: "x".repeat(1000000) }; // 1MB object
            const ref = new WeakRef(obj);
            ...
            expect(dereferenced === undefined || dereferenced !== undefined).toBe(true);
        });
    });
    describe("TTL Cache Expiration Accuracy", () => {
        ...
        test("should not leak memory with cache churn", () => {
            ...
            console.log(`💾 Memory growth: ${memoryGrowth.toFixed(2)}MB for 10,000 cache operations`);
        });
    });
    describe("Circuit Breaker Memory Protection", () => {
        ...
        test("should track memory allocation rate", () => {
            ...
            console.log(`📊 Memory allocation rate: ${rate.toFixed(2)}MB per iteration`);
        });
    ...
    describe("Memory Growth Target: <1%", () => {
        ...
        test("should maintain stable memory with repeated operations", async () => {
            ...
            const variance = samples.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) /
                samples.length;
            const stdDev = Math.sqrt(variance);
            console.log(`📊 Memory stability - Mean: ${mean.toFixed(2)}MB, StdDev: ${stdDev.toFixed(2)}MB`);
    ...
    describe("Real-World Memory Stress Test", () => {
        test("should handle 10,000 WeakRef objects without memory explosion", () => {
            ...
            console.log(`💾 Memory growth for 10,000 WeakRefs: ${growth.toFixed(2)}MB`);
        ...
        test("should handle TTL cache with 5,000 entries", () => {
            ...
            const growth = afterCreation - initialMemory;
            cache.clear();
            console.log(`💾 Memory growth for 5,000 cache entries: ${growth.toFixed(2)}MB`);
...
//# sourceMappingURL=memory-leak-detection.test.js.map