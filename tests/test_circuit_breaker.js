...
class CircuitBreakerTest {
     ...
     */
    createFailingOperation(_failureRate, _responseTime = 100) {
        return async () => {
    ...
    async testRecovery() {
        ...
        try {
            ...
            const stats = circuit.getStats();
            const hasRecoveryTransition = stats.stateChanges.some((change) => change.to === "HALF_OPEN" && change.reason.includes("Recovery"));
    ...
    printResults() {
        ...
        if (failed === 0) {
            ...
            console.log("🔥 Sistema de protección contra cascadas funcionando PERFECTAMENTE");
        }
        else {
        ...
        Object.entries(allStats).forEach(([name, stats]) => {
            console.log(`   ${name}: ${stats.state} (${stats.totalCalls} calls, ${stats.cascadesPrevented} cascades prevented)`);
        });
    }
}
...
//# sourceMappingURL=test_circuit_breaker.js.map