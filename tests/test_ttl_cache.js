...
async function testTTLCache() {
    ...
    try {
        ...
        console.log("\n🔄 TEST 1: Operaciones básicas de TTL");
        const basicCache = new TTLCache("test-basic", {
        ...
        console.log("\n🔄 TEST 2: TTL y renovación");
        const ttlCache = new TTLCache("test-ttl", {
            defaultTTL: 1500, // 1.5 segundos
            onExpire: (_key, _user) => console.log(`⏰ Usuario ${_user.name} expiró del cache`),
        });
        const user1 = { id: 1, name: "Alice", lastLogin: Date.now() };
        const user2 = { id: 2, name: "Bob", lastLogin: Date.now() };
        ...
        console.log(`⏱️ TTL user1 después de renovar: ${ttlCache.getTTL("user1")}ms`);
        // Esperar expiración de user2
        await new Promise((_resolve) => setTimeout(_resolve, 1000));
        console.log(`📖 User1 (renovado): ${ttlCache.get("user1")?.name || "EXPIRADO"}`);
        console.log(`📖 User2 (no renovado): ${ttlCache.get("user2")?.name || "EXPIRADO"}`);
        // Test 3: Límite de tamaño y evicción
        console.log("\n🚪 TEST 3: Límite de tamaño y evicción");
        const sizedCache = new TTLCache("test-sized", {
            defaultTTL: 10000, // 10 segundos (suficiente para test)
            maxSize: 3,
            onEvict: (_key, _value) => console.log(`🚪 EXPULSADO por tamaño: ${_key} = ${_value}`),
        });
        ...
        console.log("\n📊 TEST 4: Estadísticas detalladas");
        const statsCache = new TTLCache("test-stats", {
            defaultTTL: 5000,
            maxSize: 5,
        });
        // Generar actividad
        ...
        console.log("\n⏰ TEST 5: Entradas próximas a expirar");
        const expiringCache = new TTLCache("test-expiring", {
            defaultTTL: 2000,
        });
        ...
        console.log("\n🏭 TEST 6: Factory methods");
        const fastCache = TTLCacheFactory.createFastCache("fast");
        const sessionCache = TTLCacheFactory.createSessionCache("session");
        const longCache = TTLCacheFactory.createLongCache("long");
        const unlimitedCache = TTLCacheFactory.createUnlimitedCache("unlimited", 30000);
        console.log(`🏭 FACTORY CACHES CREADOS:`);
        console.log(`  ⚡ Fast: ${fastCache.getStats().maxSize} max, TTL: 5min`);
        console.log(`  👤 Session: ${sessionCache.getStats().maxSize} max, TTL: 30min`);
        console.log(`  🐌 Long: ${longCache.getStats().maxSize} max, TTL: 2h`);
        console.log(`  ∞ Unlimited: ${unlimitedCache.getStats().maxSize || "∞"} max, TTL: 30s`);
        ...
        console.log(`👤 Sesión recuperada: ${retrievedSession?.sessionId}, actividades: ${retrievedSession?.activity.length}`);
        // Test 7: Información detallada de entradas
        console.log("\n📋 TEST 7: Información detallada de entradas");
        const infoCache = new TTLCache("test-info", {
            defaultTTL: 5000,
        });
        ...
        if (entryInfo) {
            ...
            console.log(`  👁️ Último acceso: ${new Date(entryInfo.accessed).toISOString()}`);
        ...
        const debugCache = new TTLCache("debug", {
            defaultTTL: 1000,
            maxSize: 2,
        });
...
//# sourceMappingURL=test_ttl_cache.js.map