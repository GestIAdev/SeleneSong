...
describe("Deterministic End-to-End Integration Tests", () => {
    ...
    it("should complete full swarm-redis integration cycle deterministically", async () => {
        ...
        assert.strictEqual(swarmData.swarm_id, "e2e-test-swarm", "Swarm ID should match");
        assert.ok(swarmData.immortal_state, "Should have immortal state");
        assert.ok(typeof swarmData.uptime === "number", "Should have numeric uptime");
        ...
        assert.strictEqual(immortalState1.genesis_active, immortalState2.genesis_active);
        assert.strictEqual(immortalState1.democracy_operational, immortalState2.democracy_operational);
    ...
    it("should maintain deterministic behavior across multiple operations", async () => {
        ...
        assert.deepStrictEqual(results[0], results[2], "Cached immortal state should be identical");
    });
...
//# sourceMappingURL=e2e-deterministic-integration.test.js.map