...
interface TestResult {
  ...
  metrics?: {
    ...
    circularDetected?: number;
  };
}

class WeakReferenceManagerTest {
  private manager: WeakReferenceManager;
  ...
  private createTestComponent(_id: string, _name: string): TestComponent {
    return {
      _id,
      _name,
   ...
   */
  private async testBasicReferenceManagement(): Promise<TestResult> {
    ...
    try {
      ...
      const recovered = this.manager
        .getWeakRef<TestComponent>("test-basic-1")
        ?.get();
   ...
   */
  private async testComponentRelationships(): Promise<TestResult> {
    const testName = "Component Relationships";

    try {
      // Crear componentes padre e hijo
      const parent = this.createTestComponent(
        "test-parent-1",
        "Parent Component",
      );
      const child1 = this.createTestComponent(
        "test-child-1",
        "Child Component 1",
      );
      const child2 = this.createTestComponent(
        "test-child-2",
        "Child Component 2",
      );
      ...
      this.manager.createRelationship(
        "test-child-1",
        "test-parent-1",
        "dependency",
      );
      ...
      const children = this.manager.getChildren<TestComponent>("test-parent-1");
      const dependencies =
        this.manager.getDependencies<TestComponent>("test-child-1");
   ...
   */
  private async testCircularReferenceDetection(): Promise<TestResult> {
    ...
    try {
      ...
      const hasAllComponents = expectedIds.every((_id) =>
        cycle.cycle.includes(_id),
      );
  ...
  private async testGarbageCollection(): Promise<TestResult> {
    ...
    try {
      ...
      // Crear componentes temporales en un scope local
      {
        ...
        const ref1 = this.manager.getWeakRef<TestComponent>("temp-1");
        const ref2 = this.manager.getWeakRef<TestComponent>("temp-2");
   ...
   */
  private async testPerformanceWithManyReferences(): Promise<TestResult> {
    ...
    try {
      ...
      for (let i = 0; i < componentCount; i++) {
        const ref = this.manager.getWeakRef<TestComponent>(`perf-${i}`);
        if (ref?.has()) {
  ...
  private printResults(): void {
    ...
    if (failed === 0) {
      ...
      console.log(
        "🔥 Sistema de referencias débiles funcionando PERFECTAMENTE",
      );
    } else {
    ...
    console.log(
      `   Referencias recolectadas: ${finalStats.garbageCollectedRefs}`,
    );
