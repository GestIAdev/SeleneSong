...
async function testLimitedBuffer(): Promise<void> {
  ...
  try {
    ...
    const rotateBuffer = new LimitedBuffer<TestDataItem>("test-rotate", {
      maxSize: 5,
      onOverflow: "rotate",
    ...
    console.log(
      `📊 Buffer size: ${rotateBuffer.size()}, Usage: ${rotateBuffer.getUsagePercentage().toFixed(1)}%`,
    );
    console.log(
      `📋 Items: ${rotateBuffer
        .getAll()
    ...
    console.log("\n❌ TEST 2: Buffer con rechazo");
    const rejectBuffer = new LimitedBuffer<string>("test-reject", {
      maxSize: 3,
      onOverflow: "reject",
      warningThreshold: 0.8,
    });
    ...
    const compressBuffer = new LimitedBuffer<number>("test-compress", {
      maxSize: 10,
      onOverflow: "compress",
    ...
    for (let i = 1; i <= 15; i++) {
      ...
      if (i === 10)
        console.log(
          `📊 Antes de overflow: [${compressBuffer.getAll().join(", ")}]`,
        );
    }

    console.log(
      `📊 Después de compresión: [${compressBuffer.getAll().join(", ")}]`,
    );

    // Test 4: Métodos de búsqueda y filtrado
    console.log("\n🔍 TEST 4: Búsqueda y filtrado");
    const searchBuffer = BufferFactory.createEventBuffer<TestDataItem>(
      "test-search",
      20,
    );
    ...
    console.log("\n⏰ TEST 5: Limpieza por tiempo");
    const timeBuffer = BufferFactory.createLogBuffer<TestDataItem>(
      "test-time",
      10,
    );
    ...
    const removed = timeBuffer.removeOlderThan(
      3 * 60000,
      (_item) => _item.timestamp,
    );
    ...
    console.log("\n📊 TEST 6: Estadísticas y redimensionamiento");
    const statsBuffer = BufferFactory.createMetricsBuffer<number>(
      "test-stats",
      8,
    );
