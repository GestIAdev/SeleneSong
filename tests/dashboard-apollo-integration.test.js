...
describe("Dashboard-Selene Integration Tests", () => {
    ...
    describe("Selene Backend Health Check", () => {
        test("should respond to health check on port 8003", async () => {
            const response = await fetch(`http://localhost:${APOLLO_PORT}/api/health`, {
                method: "GET",
            }).catch(() => null);
            // If Selene is running, should get a response
            // If not running, test will mark as skipped
            if (!response) {
                console.warn("⚠️ Selene backend not running on port 8003 - skipping test");
                return;
            }
    ...
    describe("WebSocket Connection - Dashboard to Client", () => {
        ...
        test("should receive real-time swarm data over WebSocket", (done) => {
            ...
            ws.on("message", (data) => {
                const parsed = JSON.parse(data.toString());
                messageCount++;
                console.log(`📊 Message ${messageCount}: Timestamp ${parsed.timestamp}`);
        ...
        test("should receive metrics with correct structure", (done) => {
            ...
            ws.on("message", (data) => {
                ...
                expect(typeof parsed.immortalState.democracy_operational).toBe("boolean");
        ...
        test("should receive metrics updates at regular intervals", (done) => {
            ...
            ws.on("message", (data) => {
                ...
                if (timestamps.length >= 3) {
                    ...
                    const ratio = Math.max(interval1, interval2) / Math.min(interval1, interval2);
    ...
    describe("Dashboard Performance & Stability", () => {
        test("should handle multiple WebSocket clients simultaneously", (done) => {
            ...
            setTimeout(() => {
                clients.forEach((client) => client.close());
                console.log(`📊 Connected: ${connectedClients}/${clientCount}, Messages: ${messagesReceived}`);
                done();
            }, 10000);
        }, 15000);
...
//# sourceMappingURL=dashboard-apollo-integration.test.js.map